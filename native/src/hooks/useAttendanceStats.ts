      ...
      try {
        ...
        const breakdown = enrollments.map((e) => {
          const courseSessions =
            sessions?.filter((s) => s.course_id === e.course_id) || [];
          const courseAttended = courseSessions.filter((s) =>
            recordSet.has(s.id)
          ).length;
          const percentage =
        ...
        const attendedSess =
          sessions?.filter((s) => recordSet.has(s.id)).length || 0;
        const progress =
          totalSess > 0 ? Math.round((attendedSess / totalSess) * 100) : 100;
