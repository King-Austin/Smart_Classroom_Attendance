    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (sessionsData) {
          ...
          const totalStudents = new Set(assignments?.map((a) => a.student_id))
            .size;

          const totalPresent = sessionsWithStats.reduce(
            (acc, s) => acc + s.present,
            0
          );
          const totalPossible = sessionsWithStats.reduce(
            (acc, s) => acc + s.total,
            0
          );
          const avgRate = calculatePercentage(totalPresent, totalPossible);

          setStats({ totalStudents, courseCount: uniqueCourseIds.size, avgRate });
        }
      } catch (err) {
